...
def connect_JSON(config):
    ...
    if not 'rpcport' in config:
        config['rpcport'] = 31450 if testnet else 31050
...
def list_available(creditsd):
    ...
    for output in unspent:
        ...
        # This code only deals with ordinary pay-to-credits-address
...
def create_tx(creditsd, fromaddresses, toaddress, amount, fee):
    ...
    if total_available < needed:
        sys.stderr.write("Error, only %f CRDS available, need %f\n"%(total_available, needed));
        sys.exit(1)
...
def main():
    ...
    config = read_credits_config(options.datadir)
