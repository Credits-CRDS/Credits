...
- [Building Credits](#building-credits)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-credits-host/README.md).
...
- Storage on Physical hard drive: Creditsally Allocated 
...
Clone the git repositories for credits and gitian and then checkout the credits version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the credits repository
...
To build Credits (for Linux, OSX and Windows) just follow the steps under 'perform
...
```bash
    ...
    From https://github.com/credits-crds/credits
...
For example:
```bash
URL=https://github.com/crowning-/credits.git
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/credits-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/credits-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/credits-build.assert
```
